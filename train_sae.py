...
def train_sae(cfg): 
    ...
    for epoch_idx in range(cfg.num_epochs): 
        for batch_idx, batch in tqdm(enumerate(dataloader), total=len(dataloader)): 
            ...
            writer.add_scalar("Train/l2", recon_loss(act, act_recon).item() , glob_iter)
            writer.add_scalar("Train/l1", act_encode.abs().sum(), glob_iter)
            writer.add_scalar("Train/nnz", (act_encode.abs() > 1e-6 ).sum() / int(hidden_dim  * cfg.sparsity_coeff), glob_iter)
            energy = torch.sum(act_encode.detach().cpu()**2)
